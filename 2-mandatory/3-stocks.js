...
function getAveragePrices(closingPricesForAllStocks) {
  function getAveragePrice(subArray) {
    return Number(
      (subArray.reduce((aV, cV) => aV + cV, 0) / subArray.length).toFixed(2)
    );
  }
...
function getPriceChanges(closingPricesForAllStocks) {
  function getPriceChange(subArray) {
    return Number((subArray[subArray.length - 1] - subArray[0]).toFixed(2));
  }
  return closingPricesForAllStocks.map((element) => getPriceChange(element));
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  function findHighestPrise(subArray) {
    return Math.max(...subArray);
  }
  //   console.log(findHighestPrise(subArray));

  //   const STOCKS = ["aapl", "msft", "amzn", "googl", "tsla"];

  //   function getIndexForStocks(stocksArray) {
  //     let index = 0;
  //     for (let i = 0; i <= stocksArray.length; i++) {
  //       index += i;
  //     }
  //     return stocksArray[index];
  //   }
  //   console.log(getIndexForStocks(STOCKS));
  //   console.log(getIndexForStocks(STOCKS));
  //   console.log(getIndexForStocks(STOCKS));

  return stocks.map(
    (arr, index) =>
      `The highest price of ${arr.toUpperCase()} in the last 5 days was ${findHighestPrise(
        closingPricesForAllStocks[index]
      ).toFixed(2)}`
  );
}
