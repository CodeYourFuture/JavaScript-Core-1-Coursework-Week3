...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    let sum = 0;
    for (let index = 0; index < pricesOfEachArray.length; index++) {
...
function getPriceChanges(closingPricesForAllStocks) {
  ...
  const arrayOfchangesLastDayFromFirst = [];
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    const eachArrayPricess = closingPricesForAllStocks[i];
    const changesOfEach =
      eachArrayPricess[eachArrayPricess.length - 1] - eachArrayPricess[0];
    arrayOfchangesLastDayFromFirst.push(parseFloat(changesOfEach.toFixed(2)));
  }
  return arrayOfchangesLastDayFromFirst;
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    const capitalisedStockName = stocks[i].toUpperCase();
