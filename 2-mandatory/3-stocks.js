...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (const companyPrices of closingPricesForAllStocks) {
    ...
    const averageStockPrice =
      Math.round((companyPriceTotal / daysCount) * 100) / 100;
    stockAveragePrices.push(averageStockPrice);
  }
  return stockAveragePrices;
}
...
function getPriceChanges(closingPricesForAllStocks) {
  // TODO
  const stockPriceDifferences = [];
  for (const companyPrices of closingPricesForAllStocks) {
    const firstPricePosition = 0;
    const lastPricePosition = companyPrices.length - 1;
    const stockPriceDiff =
      companyPrices[lastPricePosition] - companyPrices[firstPricePosition];
    const stockPriceDiffRounded = Math.round(stockPriceDiff * 100) / 100;
    stockPriceDifferences.push(stockPriceDiffRounded);
  }
  return stockPriceDifferences;
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  // TODO
  const highestStockPrices = [];
  let stockCounter = 0;
  for (const companyPrices of closingPricesForAllStocks) {
    let highestPrice = 0;
    for (const singlePrice of companyPrices) {
      if (singlePrice > highestPrice) {
        highestPrice = singlePrice;
      }
    }
    let highestPriceRounded = Math.round(highestPrice * 100) / 100;
    let stockUpperCase = stocks[stockCounter].toUpperCase();
    stockCounter++;
    let highestPriceEntry = `The highest price of ${stockUpperCase} in the last 5 days was ${highestPriceRounded.toFixed(
      2
    )}`;
    highestStockPrices.push(highestPriceEntry);
  }
  return highestStockPrices;
}
