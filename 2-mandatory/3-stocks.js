...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    averagePrices.push(Number((total / closingPricesForAllStocks[i].length).toFixed(2)));
  }
...
function getPriceChanges(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    let lastDay = closingPricesForAllStocks[i][closingPricesForAllStocks.length - 1];
    let difference = lastDay - firstDay;
    priceChanges.push(Number(difference.toFixed(2)));
  }
  return priceChanges;
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  let reportStringsArray = [];
  let highestValuesArray = [];
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    let highestValue = Math.max(...closingPricesForAllStocks[i]);
    highestValuesArray.push(highestValue.toFixed(2));
    reportStringsArray.push(
      `The highest price of ${stocks[i].toUpperCase()} in the last 5 days was ${
        highestValuesArray[i]
      }`
    );
  }
  return reportStringsArray;
}
