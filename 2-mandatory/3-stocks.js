...
function getAveragePrices(closingPricesForAllStocks) {
  let priceOverTheLastFiveDaysForEachStock = [];
  closingPricesForAllStocks.forEach((stock) => {
    const total = stock.reduce((acc, stock) => {
      acc + stock, 0;
      let average = total / stock.length;
      let newAverage = average.toFixed(2);
      priceOverTheLastFiveDaysForEachStock.push(newAverage);
    });
  });

  return priceOverTheLastFiveDaysForEachStock;
}
...
function getPriceChanges(closingPricesForAllStocks) {
  ...
  let changeInPrice = [];
  closingPricesForAllStocks.forEach((stock) => {
    const priceChange = stock[stock.length - 1] - stock[0];
    changeInPrice.push(priceChange);
  });
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  for (let i = 0; i < CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS.length; i++) {
    const stockSticker = STOCKS[i];
    const prices = CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS[i];

    theHighestPrice = Math.max(...prices).toFixed(2);

    highestPrice.push(
      `The highest price of ${stockSticker} in the last 5 days was ${theHighestPrice}`
    );
  }
  return highestPrice;
}
