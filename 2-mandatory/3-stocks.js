...
function getAveragePrices(closingPricesForAllStocks) {
  // TODO: Initiate a new array
  let averagePriceOverLastFiveDays = [];
  // Iterate through input array
  for (const closingPricesForIndividualCompany of closingPricesForAllStocks) {
    let totalDailyPrices = 0;
    //For each item of input array: Iterate through array inside of item.
    for (const dailyPrices of closingPricesForIndividualCompany) {
      // add array items by letting totalDailyPrices equal totalDailyPrices + dailyPrices;
      totalDailyPrices += dailyPrices;
    }
    // find average daily price by dividing totalDailyPrices by 5
    averageDailyPrices = totalDailyPrices / 5;
    // add average price to averagePriceOverLastFiveDays array
    averagePriceOverLastFiveDays.push(
      Math.round(averageDailyPrices * 100) / 100
    );
  }
  return averagePriceOverLastFiveDays;
}
...
function getPriceChanges(closingPricesForAllStocks) {
  // TODO
  // initiate a new array priceChangeArr
  let priceChangeArr = [];
  // iterate input array
  for (const closingPricesForIndividualCompany of closingPricesForAllStocks) {
    // find price change by using index 0 and 4 to access values for Monday and Friday prices and take them away from each other
    priceChange =
    ...
    priceChangeArr.push(Math.round(priceChange * 100) / 100);
  }
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  // find number companies by checking length of array.
  let numberOfCompanies = stocks.length;
  // initiate highestPriceArr array
  let highestPricesArr = [];
  // start a loop for i between 0 and numberOfCompanies
  for (let i = 0; i < numberOfCompanies; i++) {
    let highestDailyPrice = 0;
    // iterate through array inside input array closingPricesForAllStocks
    for (dailyPrice of closingPricesForAllStocks[i]) {
      // if the dailyPrice is greater than current highest price stored in variable highestDailyPrice, assign current dailyPrice to highestDailyPrice
      if (dailyPrice > highestDailyPrice) {
        highestDailyPrice = dailyPrice;
      }
    }
    // transform string to uppercase
    stocks[i] = stocks[i].toUpperCase();
    // add string to highestPriceArr array. the highestDailyPrice value is set to 2 decimal point using toFixed method
    highestPricesArr.push(
      `The highest price of ${
        stocks[i]
      } in the last 5 days was ${highestDailyPrice.toFixed(2)}`
    );
  }
  return highestPricesArr;
  // TODO
}
