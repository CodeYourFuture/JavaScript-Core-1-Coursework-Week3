...
function potentialHeadlines(allArticleTitles) {
  const mapResult = allArticleTitles.filter((string) => string.length <= 65);
...
function headlinesWithNumbers(allArticleTitles) {
  const headlineWithNumber = allArticleTitles.filter((string) =>
    /\d/.test(string)
  );
  return headlineWithNumber;
}
...
function averageNumberOfCharacters(allArticleTitles) {
  ...
  for (let i = 0; i < allArticleTitles.length; i++) {
    totalCh += allArticleTitles[i].length;
  }
  let average = Math.round(totalCh / allArticleTitles.length);
  return average;
}
