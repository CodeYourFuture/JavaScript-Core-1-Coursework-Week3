...
function headlinesWithNumbers(allArticleTitles) {
  // TODO
  const headlinesWithNumbersArray = [];

  for (const eachTitle of allArticleTitles) {
    for (const char of eachTitle) {
      if (!isNaN(parseInt(char))) {
        headlinesWithNumbersArray.push(eachTitle);
      }
    }
  }

  return headlinesWithNumbersArray;
}
...
function averageNumberOfCharacters(allArticleTitles) {
  ...
  for (const everyTitle of allArticleTitles) {
    totalChars += everyTitle.length;
  }

  let averageNumber = totalChars / allArticleTitles.length;
