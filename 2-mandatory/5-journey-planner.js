...
function isAccessibleByTransportMode(arrTransport, string) {
  return arrTransport[0]==string[0] ? true : false
...
function journeyPlanner(locations, transportMode) {
  ...
  let filterLocation=locations.filter(arr => arr.includes(transportMode));
  let returnArray= filterLocation.map(element => element[0])
  return returnArray;
}
