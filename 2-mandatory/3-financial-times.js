...
function potentialHeadlines(allArticleTitles) {
  // TODO
  let headlineArticleTitles = [];
  for (let i = 0; i < allArticleTitles.length; i++) {
    if (allArticleTitles.length <= 65) {
      headlineArticleTitles.push(allArticleTitles[i]);
    }
  }
  return headlineArticleTitles;
}
...
function headlinesWithNumbers(allArticleTitles) {
  ...
  let stringTitle;
  for (let i = 0; i < allArticleTitles.length; i++) {
    stringTitle = allArticleTitles[i];
    for (let j = 0; j < stringTitle.length; j++) {
      if (!isNaN(parseInt(stringTitle[j]))) {
        titlesWithNumbers.push(stringTitle);
      }
    }
  }
