...
function potentialHeadlines(allArticleTitles) {
  ...
  for (let article of allArticleTitles) {
    if (article.length <= 65) fitArticleArray.push(article);
  }
  return fitArticleArray;
}
...
function titleWithFewestWords(allArticleTitles) {
  // TODO
  let allArticleTitlesLengths = [];
  for (let article of allArticleTitles) {
    allArticleTitlesLengths.push(article.split(" ").length);
  }
  let indexOfShortestTitle = allArticleTitlesLengths.indexOf(
    Math.min(...allArticleTitlesLengths)
  );
...
function averageNumberOfCharacters(allArticleTitles) {
  // TODO
  let sumArticleLengths = 0;
  let average = 0;
  for (let i = 0; i < allArticleTitles.length; i++) {
    sumArticleLengths = sumArticleLengths + allArticleTitles[i].length;
  }
  average = sumArticleLengths / allArticleTitles.length;
