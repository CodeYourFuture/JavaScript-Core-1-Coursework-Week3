...
function potentialHeadlines(allArticleTitles) {
  // TODO
  //loop through allArticleTitles - for of loop
  //find all titles <= 65 characters - .length
  //return new array with titles that fit - return ; new array
...
function titleWithFewestWords(allArticleTitles) {
  ...
  let prevWords = allArticleTitles[0].split(" ").length;

  for (let i = 1; i < allArticleTitles.length; i++) {
    let wordArray = allArticleTitles[i].split(" ").length;
    if (wordArray < prevWords) {
      fewestWords = allArticleTitles[i];
      //   console.log("This is less", allArticleTitles[i]);
    }
    prevWords = wordArray;
  }
  return fewestWords;
