...
function potentialHeadlines(allArticleTitles) {
  ...
  for (let article of allArticleTitles) {
    if (article.length <= 65) {
      shortArticleTitles.push(article);
    }
  }
...
/*function titleWithFewestWords(allArticleTitles) {
  let arrayWithFewestWords = [];
  for (let article of allArticleTitles) {
    let splitArray = article.split(" ");
    arrayWithFewestWords.push(splitArray.length);
  }
  return Math.min.apply(Math, arrayWithFewestWords);
}*/
function titleWithFewestWords(allArticleTitles) {
  let noOfWords;
  let smallestTitle;
  for (let title of allArticleTitles) {
    let splitTitle = title.split(" ").length;
    if (splitTitle < noOfWords || noOfWords === undefined) {
      noOfWords = splitTitle;
      smallestTitle = title;
    }
  }
