...
function potentialHeadlines(allArticleTitles) {
  // TODO
  let properTitles = [];
  let properTitleIndex = 0;
  for (let i = 0; i < allArticleTitles.length; i++) {
    if (allArticleTitles[i].length <= 65) {
      properTitles[properTitleIndex] = allArticleTitles[i];
      properTitleIndex++;
    }
  }
  return properTitles;
...
function titleWithFewestWords(allArticleTitles) {
  ...
  for (let i = 1; i < allArticleTitles.length; i++) {
    if (allArticleTitles[shortArticleIndex] < allArticleTitles[i])
      shortArticleIndex = i;
  }

  return allArticleTitles[shortArticleIndex];
}
...
function headlinesWithNumbers(allArticleTitles) {
  ...
  for (i = 0; i < allArticleTitles.length; i++) {
    if (pattern.test(allArticleTitles[i])) {
      resultArticle[articleIndex] = allArticleTitles[i];
      articleIndex++;
    }
  }
...
function averageNumberOfCharacters(allArticleTitles) {
  ...
  for (let i = 0; i < allArticleTitles.length; i++) {
    sumArticleChar += allArticleTitles[i].length;
  }
  averageNumber = sumArticleChar / numberOfArticles;
