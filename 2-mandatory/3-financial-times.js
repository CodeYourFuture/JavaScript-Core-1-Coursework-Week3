...
const headlinesWithNumbers = (allArticleTitles) => {
  ...
  for (let i = 0; i < allArticleTitles.length; i++) {
    if (allArticleTitles[i].match(/[0123456789]/gi)) {
      includesNum.push(allArticleTitles[i]);
    }
  }
...
const averageNumberOfCharacters = (allArticleTitles) => {
  ...
  for (article of allArticleTitles) {
    totalSum += article.length;
  }

  const averageNum = Math.round(totalSum / allArticleTitles.length);
