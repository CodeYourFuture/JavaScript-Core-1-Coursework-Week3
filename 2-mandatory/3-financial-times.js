...
function potentialHeadlines(allArticleTitles) {
  ...
  for (let allTitles of allArticleTitles)
    if (allTitlecs.length < 65) fitTitles.push(allTitles);
  return fitTitles;
}
...
function titleWithFewestWords(allArticleTitles) {
  ...
  for (let allTitles of allArticleTitles) {
    ...
    lengthTitles.push(temp);
  }

  lengthTitles.sort(function (a, b) {
    return a - b;
  });

  for (let allTitles of allArticleTitles) {
    temp = allTitles.split(" ").length;
    if (temp === lengthTitles[0]) return allTitles;
  }
}
...
function averageNumberOfCharacters(allArticleTitles) {
  ...
  let sum = 0;
  let avg = 0;
  for (let allTitles of allArticleTitles) sum += allTitles.length;
  avg = sum / allArticleTitles.length;
