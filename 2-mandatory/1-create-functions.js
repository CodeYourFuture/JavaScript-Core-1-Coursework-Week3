...
*/
function tidyUpString(string) {function tidyUpString(string) {
  return string.map(elem => elem.replace(/\//g,'').trim().toLowerCase())}
...
function remove(arr, index) {var newArr = arr.slice(0, arr.length);
...
function formatPercentage(array) {return array.map((number) =>
