...
function getAveragePrices(closingPricesForAllStocks) {
	...
	for (prices of closingPricesForAllStocks) {
		...
		averages.push(
			Math.round((total / prices.length + Number.EPSILON) * 100) / 100
		);
	}
...
function getPriceChanges(closingPricesForAllStocks) {
	...
	for (prices of closingPricesForAllStocks) {
		priceChanges.push(
			Math.round(
				(prices[prices.length - 1] - prices[0] + Number.EPSILON) * 100
			) / 100
		);
	}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
	...
	stocks.forEach((company, i) => {
		highestPrices.push(
			`The highest price of ${company.toUpperCase()} in the last 5 days was ${Math.max(
