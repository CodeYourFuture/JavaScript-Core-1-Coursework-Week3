...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    for (let j = 0; j < closingPricesForAllStocks[i].length; j++) {
      stocksSum += closingPricesForAllStocks[i][j];
    }
    avgSum = stocksSum / closingPricesForAllStocks[i].length;
    let finalNum = avgSum.toFixed(2);
    averagesArray.push(parseFloat(finalNum));
  }
...
function getPriceChanges(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    let tot = closingPricesForAllStocks[i].length - 1;
    let diffSum =
      closingPricesForAllStocks[i][tot] - closingPricesForAllStocks[i][0];

    differenceArray.push(parseFloat(diffSum.toFixed(2)));
  }
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    let highestTot = 0;
    for (let j = 0; j < closingPricesForAllStocks[i].length; j++) {
      if (closingPricesForAllStocks[i][j] > highestTot) {
        highestTot = closingPricesForAllStocks[i][j];
      }
    }
    highStockPrices.push(
      `The highest price of ${stocks[
        i
      ].toUpperCase()} in the last 5 days was ${highestTot.toFixed(2)}`
    );
  }

  return highStockPrices;
}
