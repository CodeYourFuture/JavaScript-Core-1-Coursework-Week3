...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  let ArraySentenceStock = [];
  let highestStock = [];
  let stockNameCapitalised = [];

  for (let stockPrice of closingPricesForAllStocks) {
    let maxStockPrice = 0;
    for (let compareStockPrice of stockPrice) {
      if (compareStockPrice > maxStockPrice) {
        maxStockPrice = compareStockPrice;
      }
    }

    highestStock.push(maxStockPrice.toFixed(2));
  }

  for (let stockName of stocks) {
    stockNameCapitalised.push(stockName.toUpperCase());
  }

  for (let i = 0; i < stocks.length; i++) {
    let sentence = `The highest price of ${stockNameCapitalised[i]} in the last 5 days was ${highestStock[i]}`;
    ArraySentenceStock.push(sentence);
  }

  return ArraySentenceStock;
}
