...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    for (let j = 0; j < closingPricesForAllStocks[i].length; j++) {
      avg = avg +=
        closingPricesForAllStocks[i][j] / closingPricesForAllStocks[i].length;
    }
    avgArr.push(parseFloat(avg.toFixed(2)));
  }
  return avgArr;
}
...
function getPriceChanges(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    priceChanges.push(roundedDifference);
  }
  return priceChanges;
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  for (
    let i = 0;
    i < closingPricesForAllStocks.length && i < stocks.length;
    i++
  ) {
    let max = 0;
    max = Math.max.apply(null, closingPricesForAllStocks[i]);
    maxArr.push(
      `The highest price of ${stocks[
        i
      ].toUpperCase()} in the last 5 days was ${max.toFixed(2)}`
    );
  }
  return maxArr;
}
