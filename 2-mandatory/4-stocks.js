...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (let stock of closingPricesForAllStocks) {
    ...
    averagePriceArray.push(Math.round(average * 100) / 100);
  }
...
function getPriceChanges(closingPricesForAllStocks) {
  ...
  for (let closingPrices of closingPricesForAllStocks) {
    let change = ((closingPrices[closingPrices.length - 1]) - (closingPrices[0]));
    changeInPrice.push((Math.round(change*100))/100);
  }
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  // TODO
  let highestPrice = [];
  let highestPriceWithStock = [];
  for (let closingPrices of closingPricesForAllStocks) {
    highestPrice.push(Math.max(...closingPrices));
  }
  for(let i=0; i<stocks.length; i++) {
    highestPriceWithStock.push(
      `The highest price of ${stocks[
        i
      ].toUpperCase()} in the last 5 days was ${highestPrice[i].toFixed(2)}`
    );
  }
  return highestPriceWithStock;
}
