...
function getAveragePrices(closingPricesForAllStocks) {
  let averagePricesForALLStocks = []; //create the
  //map through??
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    let closingPricesForStock = closingPricesForAllStocks[i]; // creat a new variable

    //this is the total price for the day
    let totalValue = 0;
    for (let closingPricesOfDay of closingPricesForStock) {
      totalValue += closingPricesOfDay;
    }
    //get the average for the week
    averagePricesForALLStocks.push(
      Number((totalValue / closingPricesForStock.length).toFixed(2))
    );
  }
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    highest = 0;
    for (value of closingPricesForAllStocks[i]) {
      highest = value > highest ? value : highest; // update highest to the greatest to the higher value
    }
    reports.push(
