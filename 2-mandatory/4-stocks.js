...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    for (let j = 0; j < closingPricesForAllStocks[i].length; j++) {
      average += closingPricesForAllStocks[i][j];
    }
    listOfAverage[i] = parseFloat(
      (average / closingPricesForAllStocks[i].length).toFixed(2)
    );
  }
...
function getPriceChanges(closingPricesForAllStocks) {
  ...
  let priceChange = [];
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    const lastDayIndex = closingPricesForAllStocks[i].length - 1;
    const firstDayPrice = closingPricesForAllStocks[i][0];
    const lastDayPrice = closingPricesForAllStocks[i][lastDayIndex];
    priceChange[i] = parseFloat((lastDayPrice - firstDayPrice).toFixed(2));
  }
  return priceChange;
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  let price = 0.0;
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    price = closingPricesForAllStocks[i][0];

    for (let j = 1; j <= closingPricesForAllStocks[i].length; j++) {
      if (closingPricesForAllStocks[i][j] > price) {
        price = closingPricesForAllStocks[i][j];
      }
    }
    highestPice[i] = `The highest price of ${stocks[
      i
    ].toUpperCase()} in the last 5 days was ${price.toFixed(2)}`;
  }
  return highestPice;
}
