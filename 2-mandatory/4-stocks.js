...
function getAveragePrices(closingPricesForAllStocks) {
  let calcAverage = [];
  for (let pricesOfStock of closingPricesForAllStocks) {
    const average = calculateAverage(pricesOfStock); 
    const formattedAverage = Number(average.toFixed(2));
    calcAverage.push(formattedAverage);
  }
  return calcAverage;
}

function calculateAverage(stockPrices) {
  let sumOfEachStocks = 0;
  for (let price of stockPrices) {
    sumOfEachStocks += price;
  }
  const averageOfEachStocks = sumOfEachStocks / stockPrices.length;
  return averageOfEachStocks;
} 
...
function getPriceChanges(closingPricesForAllStocks) {
  let stockPrices = [];
  for (let firstPriceOfStock of closingPricesForAllStocks){
    let stockPriceChange = firstPriceOfStock[firstPriceOfStock.length - 1] - firstPriceOfStock[0];
    stockPrices.push(Number(stockPriceChange.toFixed(2)));
  }
  return stockPrices;
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  let listOfHighestPrice = [];
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    let highestPrice = Math.max(...closingPricesForAllStocks[i]);
    listOfHighestPrice.push(`The highest price of ${stocks[i].toUpperCase()} in the last 5 days was ${highestPrice.toFixed(2)}`)
  }
  return listOfHighestPrice;
}
