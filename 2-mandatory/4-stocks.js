...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    averPriceArr.push(+(sum / closingPricesForAllStocks[i].length).toFixed(2));
  }
  return averPriceArr;
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  let maxPrice = [];
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    maxPrice.push(Math.max(...closingPricesForAllStocks[i]));
    report.push(
      `The highest price of ${stocks[
        i
      ].toUpperCase()} in the last 5 days was ${maxPrice[i].toFixed(2)}`
    );
  }
  return report;
}
