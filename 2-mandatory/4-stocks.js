...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (let stockValue of closingPricesForAllStocks) {
    let total = stockValue.reduce((oldPrice, newPrice) => {
      return oldPrice + newPrice;
    });
    let avgPrice = (total / 5).toFixed(2);
    result.push(Number(avgPrice));
  }
  return result;
}
...
function getPriceChanges(closingPricesForAllStocks) {
  // TODO
  let result = [];
  closingPricesForAllStocks.forEach((price) => {
    result.push(Number((price[price.length - 1] - price[0]).toFixed(2)));
  });
  return result;
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    highest.push(Math.max(...closingPricesForAllStocks[i]));
    stockToUpper = stocks[i];
    financialReport.push(
      "The highest price of " +
        stockToUpper.toUpperCase() +
