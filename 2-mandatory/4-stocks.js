...
const CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS = [
  ...
  [1101.31, 1093.94, 1067.0, 1008.87, 938.53], // TSLA
];
...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  let average;
  let averagePrices = [];
  let pricesArrayLength = CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS.length;
  for (let i = 0; i < STOCKS.length; i++) {
    let sumPrices = 0;
    for (let j = 0; j < pricesArrayLength; j++) {
      sumPrices += CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS[i][j];
    }
    average = sumPrices / pricesArrayLength;
    averagePrices[i] = Number(average.toFixed(2));
  }
...
function getPriceChanges(closingPricesForAllStocks) {
  // TODO
  let weekOpeniningPrice;
  let weekClosingPrice;
  let changeInPrice;
  let stockPricesChange = [];
  for (let i = 0; i < STOCKS.length; i++) {
    weekOpeniningPrice = CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS[i][0];
    weekClosingPrice = CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS[i][4];

    changeInPrice = weekClosingPrice - weekOpeniningPrice;
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  // TODO
  let stocksHighestPrices = [];
  let pricesArrayLength = CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS.length;
  for (let i = 0; i < STOCKS.length; i++) {
    let highestPrice = 0;
    let highestPriceindex = 0;
    for (let j = 0; j < pricesArrayLength; j++) {
      if (CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS[i][j] > highestPrice) {
        highestPriceindex = j;
        highestPrice = CLOSING_PRICES_LAST_5_DAYS_FOR_ALL_STOCKS[i][j];
      }
    }
    stocksHighestPrices[i] = `The highest price of ${STOCKS[
      i
    ].toUpperCase()} in the last 5 days was ${Number(highestPrice.toFixed(3))}`;
  }
  console.log(stocksHighestPrices, "-------------------->stockHighestPrices");
  return stocksHighestPrices;
}
...
test("should return a description of the highest price for each stock", () => {
  expect(
    ...
    "The highest price of TSLA in the last 5 days was 1101.31",
  ]);
});
