...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (closingPrices of closingPricesForAllStocks) {
    let sum = 0;
 
    for (price of closingPrices) {
      sum += price;
    }
    let avaragePrices = sum / closingPrices.length;
    newArray.push(parseFloat(averagePrices.toFixed(2)));
  }
  return newArray;
}
...
function getPriceChanges(closingPricesForAllStocks) {
   ...
   for (prices of closingPricesForAllStocks) {
     let difference = item[item.length - 1] - item[0]; 
     newArray.push(parseFloat(diff.toFixed(2)));
   }
   return newArray;
}
