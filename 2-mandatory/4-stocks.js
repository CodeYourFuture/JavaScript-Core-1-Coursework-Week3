...
function getAveragePrices(closingPricesForAllStocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    newArray.push(Number(arrAveragePrice(closingPricesForAllStocks[i])));
  }
  return newArray;
}

function arrAveragePrice(closingPricesForAllStocks) {
  let total = 0;
  for (let num = 0; num < closingPricesForAllStocks.length; num++) {
    total += closingPricesForAllStocks[num];
  }
  const averageOfOneArray = (total / closingPricesForAllStocks.length).toFixed(
    2
  );
  return averageOfOneArray;
}
...
function getPriceChanges(closingPricesForAllStocks) {
  let newArray = [];
  for (let num = 0; num < closingPricesForAllStocks.length; num++) {
    //console.log(closingPricesForAllStocks[num])
    newArray.push(
      parseFloat(
        (
          closingPricesForAllStocks[num][4] - closingPricesForAllStocks[num][0]
        ).toFixed(2)
      )
    );
  }
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
  ...
  for (let i = 0; i < closingPricesForAllStocks.length; i++) {
    ...
    highestStockPrice.push(
      `The highest price of ${stocks[
        i
      ].toUpperCase()} in the last 5 days was ${highestNum.toFixed(2)}`
    );
  }
