...
function getAveragePrices(closingPricesForAllStocks) {
    // let arr = [];
    let averagePrices = closingPricesForAllStocks
        .map((el) => {
            // console.log(el);
            return (
...
function getPriceChanges(closingPricesForAllStocks) {
    let PriceChanges = closingPricesForAllStocks.map((el) =>
        Number((el[el.length - 1] - el[0]).toFixed(2).replace(/(\.0+|0+)$/, ""))
    );
    return PriceChanges;
}
...
function highestPriceDescriptions(closingPricesForAllStocks, stocks) {
    ...
    for (let i = 0; i < stocks.length; i++) {
        result.push(
            `The highest price of ${capitalisedSTocks[i]} in the last 5 days was ${highestPrice[i]}`
        );
    }
    return result;
}
