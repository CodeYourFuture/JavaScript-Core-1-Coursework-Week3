...
function getHighestRatedInEachGenre(books) {
  let topBook = {}; //create new object
  for (let book of books) {
    //check for each element of BOOKS array
    if (!topBook[book.genre] || topBook[book.genre].rating < book.rating) {
      //Check either condition:
      //1) if genre is not present on topBook object?
      //2) if rating in topBook larger than rating for the book of BOOKS array?
      topBook[book.genre] = {
        title: book.title,
        rating: book.rating,
      }; //creating topBook === object of objects
    }
  }
  return Object.values(topBook).map((e) => e.title);
  // Object.values() returns values of the objects as an array of objects .map returns only the title attribute
}
