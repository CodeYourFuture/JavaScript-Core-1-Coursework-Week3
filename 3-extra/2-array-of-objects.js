...
function getHighestRatedInEachGenre(books) {
  ...
  for (let book of BOOKS) {
    if (book.genre === "children") {
      childrenRatings.push(book.rating);
      childrenTitles.push(book.title);
    } else if (book.genre === "non-fiction") {
      nonFicRatings.push(book.rating);
      nonFicTitles.push(book.title);
    } else {
      cookingRatings.push(book.rating);
      cookingTitles.push(book.title);
    }
  }
  highestRates.push(
    childrenTitles[childrenRatings.indexOf(Math.max(...childrenRatings))]
  );
  highestRates.push(
    nonFicTitles[nonFicRatings.indexOf(Math.max(...nonFicRatings))]
  );
  highestRates.push(
    cookingTitles[cookingRatings.indexOf(Math.max(...cookingRatings))]
  );
  return highestRates;
}
